...
- `BACKLOG.md`는 아직 active plan으로 분해하지 않은 후속 과제와 보류 항목의 canonical backlog로 취급한다.
...
- canonical 문서(`docs/`, `TODO.md`, `MILESTONE.md`, `BACKLOG.md`, `docs/internal/status.md`, `docs/implementation/active/`)는 루트 저장소에서 관리하는 것을 기본 원칙으로 한다.
...
- 구현 완료 뒤 기본 closeout 경로는 `python3 scripts/implementation_docs.py closeout-plan <plan_id>`이다.
- 수동 복구가 필요한 경우에만 `archive-plan`, `sync-indexes`, `validate`를 개별 실행한다.
...
- `TODO.md`, `MILESTONE.md`, `docs/internal/status.md`의 managed block은 수동 편집하지 않고 `sync-indexes`로 갱신한다.
