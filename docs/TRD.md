...
- **6.1 Observability & Tracing (관측성)**
    ...
    - Prompt Hub 자산은 `LANGSMITH_EVAL_PROMPT_IDENTIFIER`, `LANGSMITH_MEMORY_PROMPT_IDENTIFIER`를 통해 태그 기준(`:production`, `:staging`)으로 선택하고, 조회 실패 시 로컬 fallback prompt를 사용합니다.
