...
- **내부 운영 패널 (`/internal`):**
    - 운영자는 핵심 문서(`AGENTS.md`, `CONTEXT`, `TRD`, `PRD`)를 한 곳에서 보고 필요 시 수정 동선까지 접근할 수 있어야 합니다.
    - 최근 작업, 현재 진행 상황, 다음 action, backlog가 한눈에 보여야 합니다.
    - 각 작업 항목에는 “어디서 직접 확인할 수 있는지”를 나타내는 UI 확인 경로가 함께 제공되어야 합니다.
    - 에이전트가 작업을 마친 뒤 운영 패널에 반영할 수 있도록 markdown 기반 workboard를 canonical 상태판으로 유지해야 합니다.
...
5. **[Internal Ops]** 운영자는 `/internal`에 들어가 문서, 최근 작업, 다음 action, backlog, 검증 링크를 보고 현재 제품 상태와 최근 변경을 빠르게 파악합니다.
...
- **Operational Visibility:** 운영 패널에서 최근 작업, 현재 상태, 다음 action, 핵심 문서 링크를 보고 “무엇이 바뀌었는지”를 즉시 파악할 수 있는 정도.

## 5. Internal Operations Panel Notes

- 운영 패널은 단순 관리자 페이지가 아니라 제품/운영/개발 상태를 연결하는 내부 cockpit 역할을 해야 합니다.
- 우선순위가 높은 운영 패널 콘텐츠:
    - 핵심 문서 레지스트리 및 수정 동선
    - 최근 작업 로그와 현재 진행 상태
    - 다음 action과 backlog
    ...
    - UI 검증 링크와 확인 메모
- 향후 확장 아이디어:
    - scraper/platform별 health 요약
    - 최근 파이프라인 실행 결과와 실패 로그 요약
    - 문서별 마지막 업데이트 시간, owner, 변경 이유 노출
    - 에이전트가 남긴 change summary를 자동 카드화
