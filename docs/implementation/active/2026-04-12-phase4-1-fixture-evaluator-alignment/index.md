---
plan_id: 2026-04-12-phase4-1-fixture-evaluator-alignment
title: Phase 4-1 보완: job_eval_gold.json 한국어 공고 기준 정렬
status: active
milestone: Phase 3: PromptOps 자동화 및 에이전트 운영 확장
source_agent: claude
created_at: 2026-04-12T18:00:00+09:00
updated_at: 2026-04-12T18:00:00+09:00
---
# Phase 4-1 보완: job_eval_gold.json 한국어 공고 기준 정렬

Phase 4-1에서 docs 텍스트 한국어화는 완료되었으나, `src/agent/evals/fixtures/job_eval_gold.json` 샘플 교체와 evaluator expectation 한국어화는 미완료 상태로 archive됨. 이 plan은 그 미완료 범위만을 대상으로 한다.

선행 archive 문서: `docs/implementation/archive/2026/2026-04-12-phase4-1-promptops-korean-experiment-migration/`

## 문서 구성

- [목표](01-목표.md)
- [범위](02-범위.md)
- [중요 계약](03-중요-계약.md)
- [검증](04-검증.md)
- [실행 메모](05-실행-메모.md)
