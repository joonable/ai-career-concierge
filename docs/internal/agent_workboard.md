...
- 운영 패널 전용 문서 계약인 `docs/internal/operations_panel.md`를 추가

## 현재 작업 상태

- 현재는 운영 패널 문서 계약과 작업 보드 문서를 정의한 상태
- `/internal`에서 바로 활용할 수 있도록 관련 링크를 `docs/internal/status.md`에 연결한 상태
- 문서 계약은 먼저 정리됐고, 실제 UI 확장은 다음 구현 작업으로 이어질 수 있는 상태

## 다음 action

- `/internal` 운영 허브가 `docs/internal/agent_workboard.md`를 읽어 최근 작업/다음 action/backlog를 별도 카드로 보여주도록 연결
