...
- `docs/internal/status.md`는 최근 작업, 현재 상태, 다음 action, backlog의 운영용 현재 상태를 기록합니다.
...
- 의미 있는 작업을 마치면 `docs/internal/status.md`를 업데이트해 `/internal` 운영 패널에서 최신 작업 맥락이 보이게 유지하세요.
